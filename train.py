...
X_train, X_test, y_train, y_test = train_test_split(
    ...
    test_size=0.3,
    random_state=125,
    stratify=y_encoded,  # Use stratify with encoded labels
)
...
model.fit(
    X_train, y_train, epochs=200, batch_size=16, validation_split=0.1, verbose=0
...
def representative_data_gen():
    ...
    for i in range(min(100, len(X_train))):
        ...
        sample = X_train[i]
        yield [
            sample.astype(np.float32).reshape(1, -1)
        ]  # Reshape sample to (1, num_features)
...
keras_predicted_labels = np.vectorize(reverse_label_mapping.get)(
    keras_predictions_indices
)
